# Flatpak configuration and declarative app installs
{
  ...
  home.activation.flatpakFlathub = lib.mkIf pkgs.stdenv.isLinux (lib.hm.dag.entryAfter ["writeBoundary"] ''
    ${pkgs.flatpak}/bin/flatpak --user remote-add --if-not-exists flathub https://dl.flathub.org/repo/flathub.flatpakrepo
  '');

  home.activation.flatpakTelegram = lib.mkIf pkgs.stdenv.isLinux (lib.hm.dag.entryAfter ["flatpakFlathub"] ''
    ${pkgs.flatpak}/bin/flatpak --user install -y --noninteractive flathub org.telegram.desktop
  '');

  home.activation.flatpakHeadlamp = lib.mkIf pkgs.stdenv.isLinux (lib.hm.dag.entryAfter ["flatpakFlathub"] ''
